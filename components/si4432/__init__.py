...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Required(CONF_CS_PIN): cv.int_,  # ← solo validamos que sea int
})
...
async def to_code(config):
    ...
    # Creamos un dict como espera gpio_pin_expression
    cs_pin_conf = {
        'number': config[CONF_CS_PIN],
    }
    cs = await cg.gpio_pin_expression(cs_pin_conf)
